...
> 这里推荐可以使用obsidian（基于本地文件夹，所有笔记都是 `.md` 文件。支持**双向链接**，能形成知识网络。插件生态极其强大，可无限扩展）和typora（所见即所得，界面干净，专注于写作本身。是培养 Markdown 手感和兴趣的最佳选择）作为使用软件，或者在sublime text和vscode等中使用插件。
...
4. 主流平台支持：大部分平台都支持markdown比如GitHub、Gitee、CSDN、Notion、Typora、Obsidian等等。

5. 版本控制友好：由于是纯文本格式，Markdown 文件可以很好地与 Git 等版本控制系统配合，而Word 只会将内容锁定在专有文件格式中。
...
> **提示**：在 Obsidian 中，使用 `Ctrl+1` 到 `Ctrl+6` 可快速创建相应级别的标题。有一些软件可能不支持代替语法了，不过目前来看obsidian可以。
...
> 引用中包含其他元素并非所有软件都支持显示（比如obsidian就不可以）。
...
> **注意**：锚点链接中的空格通常用连字符替代，特殊字符可能被忽略。在obsidian中可以直接用[[Markdown指南#标题]]来自动寻找要跳转的标题。
...
支持的语言包括：javascript、html、css、python、java、sql等。
...
在markdown中一些符号不能直接显示，可以使用 `\` 转义特殊字符：
...
> **注意**：并非所有 HTML 标签在所有软件中都有效，不过使用内嵌HTML能让我们实现更美观的展示效果。
...
[Obsidian数学块语法](../../../笔记/工具/Obsidian数学块语法.md)
